...
description: jest를 활용한 테스트 시 외부 저장소를 활용할 때 자주 발생하는 경고를 해결하는 방법을 정리했습니다.
category: Nodejs
tag: [jest, nodejs]
...
1. --detectOpenHandles 옵션을 사용해서 열려있는 핸들 찾기
2. 핸들을 종료하는 코드를 afterAll 블록에 추가
3. 해당 테스트 시나리오를 통합테스트로 변경
...
### 해결 방법 2. redisClient 연결 종료 코드 추가

&nbsp; &nbsp; 두 번째 방법은 redisClient 연결을 종료하는 코드를 추가하는 것입니다.
afterAll 구문을 활용하여 테스트 대상이 사용하는 redisClient 연결을 종료하는 코드를 추가하면 됩니다.
...
__mocks__ 폴더 아래에 모의 객체와 관련된 모듈을 만들고 jest에서 해당 모듈을 대신 호출하도록 할 수 있습니다.<br/>
