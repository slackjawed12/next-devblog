...
&nbsp; &nbsp; $x_{0}={pi}[i-1]=N_{0}$ 이라는 것은 $i$ 이전의 $N_{0}$개와 $x_{0}$이전의 $N_{0}$개가 같다는 것이다.
불일치한 경우에 이 $N_{0}$개에 대해 i-x 대응 관계를 반복해보면, 동일한 관계에 의해 $ {pi}[x_{0}-1]=N*{1}$ 개에 대해서는 $i$ 이전의 $N*{1}$개와 $x_{1}$ 이전의 $N_{1}$개는 같다는 결론을 얻을 수 있다.
이런 사고의 흐름이 결국 문자열 탐색에서도 동일하게 나타난다.  
